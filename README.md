...
#### Precision (정밀도) - "찾은 것 중 맞는 비율"

```
Precision = TP / (TP + FP)
```

- 예: Precision 20% → LLM이 보고한 이슈 중 **20%만 실제 이슈**, 나머지 80%는 오탐
- 의미: "이 리뷰어가 뭔가 지적하면 맞을 확률"
- 낮으면: 오탐(False Positive)이 많아 리뷰 신뢰도 하락

#### Recall (재현율) - "실제 이슈 중 찾은 비율"

```
Recall = TP / (TP + FN)
```

- 예: Recall 25% → 실제 이슈 20개 중 **5개만 발견**, 나머지 15개 놓침
- 의미: "실제 버그가 있을 때 잡아낼 확률"
- 낮으면: 중요한 버그를 놓칠 가능성 높음
...
- 둘 다 높아야 F1이 높음 (한쪽만 높으면 F1 낮음)
- 단일 숫자로 모델 성능을 비교할 때 유용

#### 결과 해석 예시

| Variant | Precision | Recall | 해석 |
|---------|-----------|--------|------|
| A | 50% | 10% | 지적은 정확하지만 너무 보수적, 많이 놓침 |
| B | 10% | 50% | 많이 찾지만 오탐 폭발, 신뢰도 낮음 |
| C | 30% | 30% | 균형 잡힘, F1 기준 A, B보다 나음 |

#### 목표 수치 (참고)

프로덕션 배포를 위한 권장 수치:
- **Precision**: 50%+ (지적한 것의 절반 이상은 맞아야 신뢰 확보)
- **Recall**: 40%+ (중요 버그의 절반 정도는 잡아야 유용함)
- **F1**: 45%+ (균형 잡힌 성능)
